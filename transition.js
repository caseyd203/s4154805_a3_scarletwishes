(() => {
  ...
  async function shellNavigate(url) {
    ...
    // index.html now contains the cover directly. Once the first scene has
    // loaded behind the fully closed wipe, swap from the built-in cover to the
    // persistent scene iframe. This keeps the original single audio owner alive.
    const builtInCover = document.getElementById('coverStage');
    if (builtInCover) builtInCover.remove();
    const coverInterface = document.getElementById('scarletInterface');
    if (coverInterface) coverInterface.remove();
    sceneFrame.style.visibility = 'visible';
    sceneFrame.style.pointerEvents = 'auto';
